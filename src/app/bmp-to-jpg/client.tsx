...
export default function BmpToJpgClient() {
    ...
    const convertToJpg = async () => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                await new Promise<void>((resolve, reject) => {
                    img.onload = () => resolve()
                    img.onerror = () => reject(new Error('Failed to load BMP'))
                })
        ...
        if (results.length === 0 && files.length > 0) {
            setError("Could not convert files. Make sure they are valid BMP images.")
        }
    ...
    if (converted.length > 0) {
        return (
            <div className="container mx-auto py-12 max-w-4xl px-4 text-center">
                ...
                <Button size="xl" onClick={downloadAll} className="bg-amber-600 hover:bg-amber-700 mb-4">
            ...
            {files.length === 0 ? (
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-xl border p-6">
                        ...
                        <Button size="xl" onClick={convertToJpg} disabled={isProcessing} className="w-full bg-amber-600 hover:bg-amber-700">
                            {isProcessing ? (
                    ...
                    <div className="text-center">
                        <FileUploader onFilesSelected={handleFilesSelected} multiple={true} accept={{ "image/bmp": [".bmp"] }} />
                    </div>
                </div>
            )}
        </div>
